...
export default async function PublicProfilePage({ params }: PublicProfilePageProps) {
  ...
  return (
    <div className="min-h-screen bg-surface-1 text-foreground">
      <header className="border-b border-border bg-surface-2 px-4 sm:px-8 py-4 flex items-center justify-between sticky top-0 z-50 pt-safe">
        <Link href="/" className="flex items-center gap-2">
          <Image src="/DevTrackAI_logo.png" alt="DevTrack AI" width={24} height={24} className="rounded-full object-cover" />
          <span className="text-[14px] font-semibold">DevTrack AI</span>
        </Link>
        <Link href="/" className="text-[13px] font-medium text-brand hover:text-brand-hover transition-colors">
          Get your portfolio →
        </Link>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-10">
        {/* Profile Hero */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 p-4 sm:p-8 border border-border bg-card rounded-[14px]">
          {user.image ? (
            <Image
              src={user.image}
            ...
            <div className="w-18 h-18 rounded-[14px] bg-muted border border-border flex items-center justify-center text-text-muted-custom font-bold text-2xl">
              {(user.name || username)[0].toUpperCase()}
            </div>
          )}
          <div className="flex-1">
            <h1 className="text-[22px] font-bold tracking-tight">{user.name || username}</h1>
            <a
              href={`https://github.com/${username}`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 text-[13px] text-text-muted-custom hover:text-brand transition-colors mt-1"
            >
            ...
            {hasAnalysis && (
              <div className="flex flex-wrap gap-3 mt-4">
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-brand-muted border border-brand-border text-brand text-[12px] font-medium">
                  <Award className="w-3.5 h-3.5" />
                  {user.portfolioAnalysis!.careerLevel} {user.portfolioAnalysis!.primaryRole}
                </span>
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-muted border border-border text-muted-foreground text-[12px] font-medium">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            <div key={label} className="p-5 border border-border bg-card rounded-[14px]">
              <div className="flex items-center gap-2 mb-2">
                <Icon className="w-4 h-4 text-text-muted-custom" />
                <span className="text-[11px] text-text-muted-custom uppercase tracking-wide">{label}</span>
              </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="p-6 border border-border bg-card rounded-[14px] space-y-4">
              ...
              <ul className="space-y-2.5">
                {(user.portfolioAnalysis!.strengths as string[]).map((s, i) => (
                  <li key={i} className="flex gap-2 items-start text-[13px] text-muted-foreground">
            ...
            <div className="p-6 border border-border bg-card rounded-[14px] space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-[14px] font-semibold">Growth Areas</h3>
              </div>
              <ul className="space-y-2.5">
        ...
        {pinnedRepos.length >= 1 && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-[16px] font-semibold">Pinned</h2>
            </div>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {pinnedRepos.map((repo) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group p-5 border border-border bg-card rounded-[14px] space-y-3 hover:border-border-medium transition-colors"
                >
                  <div className="flex items-start justify-between gap-2">
                    <span className="text-[13px] font-semibold group-hover:text-brand transition-colors truncate">
                      {repo.name}
                    </span>
                    <ExternalLink className="w-3.5 h-3.5 text-text-dim shrink-0 mt-0.5" />
                  </div>
                  {repo.description && (
                    <p className="text-[12px] text-text-muted-custom leading-relaxed line-clamp-2">{repo.description}</p>
                  )}
                  <div className="flex items-center gap-3 text-[11px] text-text-muted-custom">
                    ...
                    {repo.insights && (
                      <span className="ml-auto text-brand font-medium">{repo.insights.codeQualityScore}/100</span>
                    )}
                  </div>
                </a>
              ))}
        ...
        {user.repositories.length > 0 && (
          <div className="space-y-4">
            <h2 className="text-[16px] font-semibold">Top Repositories</h2>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <footer className="border-t border-border py-8 text-center text-[12px] text-text-dim">
        <p>
          Powered by{" "}
          <Link href="/" className="text-brand hover:text-brand-hover">DevTrack AI</Link>
